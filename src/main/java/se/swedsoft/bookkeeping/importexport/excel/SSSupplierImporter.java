...
public class SSSupplierImporter {
    ...
    private void getColumnIndexes(SSExcelRow iColumns) {
        ...
        for (SSExcelCell iColumn : iColumns.getCells()) {
            ...
            if(iName != null && iName.length() > 0) {
                ...
                if( iName.equalsIgnoreCase( SSSupplierExporter.LEVERANTÖRSNUMMER    ) ) this.iColumns.put(SSSupplierExporter.LEVERANTÖRSNUMMER      , iIndex); else
                ...
                if( iName.equalsIgnoreCase( SSSupplierExporter.VÅRT_KUNDNUMMER      ) ) this.iColumns.put(SSSupplierExporter.VÅRT_KUNDNUMMER        , iIndex); else
    ...
    private List<SSSupplier> importSuppliers(SSExcelSheet pSheet ){
        ...
        for(int row = 1; row < iRows.size(); row++){
            ...
            for(int col = 0; col < iCells.size(); col++){
                ...
                if( iColumns.containsKey(SSSupplierExporter.LEVERANTÖRSNUMMER   ) && iColumns.get(SSSupplierExporter.LEVERANTÖRSNUMMER  ) == col ) iSupplier.setNumber ( iValue );
                ...
                if( iColumns.containsKey(SSSupplierExporter.VÅRT_KUNDNUMMER     ) && iColumns.get(SSSupplierExporter.VÅRT_KUNDNUMMER    ) == col ) iSupplier.setOurCustomerNr ( iValue );
    ...
    private boolean showImportReport(List<SSSupplier> iSuppliers){
        ...
        sb.append("<ul>");
        if(iColumns.containsKey(SSSupplierExporter.LEVERANTÖRSNUMMER    )) sb.append("<li>").append(SSSupplierExporter.LEVERANTÖRSNUMMER).append("</li>");
        ...
        if(iColumns.containsKey(SSSupplierExporter.VÅRT_KUNDNUMMER      )) sb.append("<li>").append(SSSupplierExporter.VÅRT_KUNDNUMMER).append("</li>");
